...
void ClipperAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer&)
{
    ...
    // for any block size the host throws at us.
        if (numSamples > (int) thresholdBuffer.size()) thresholdBuffer.resize ((size_t) numSamples, 1.0f);
        if (numSamples > (int) mixBuffer.size()) mixBuffer.resize ((size_t) numSamples, 1.0f);
        if (numSamples > (int) makeupBuffer.size()) makeupBuffer.resize ((size_t) numSamples, 1.0f);
